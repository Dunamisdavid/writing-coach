...
const TUTORS: Tutor[] = [
    {
        ...
        icon: '🌍',
        voiceName: 'Sadachbia',
        style: 'You have a warm, friendly, conversational style. You correct kindly and keep the conversation relaxed and natural.',
    },
    {
        id: 'maya',
        name: 'Maya',
        icon: '😊',
        voiceName: 'Kore',
        style: 'You are warm, patient, and encouraging. You correct gently and celebrate small wins.',
    },
    {
        id: 'jordan',
        name: 'Jordan',
        icon: '💼',
        voiceName: 'Charon',
        style: 'You are direct and businesslike, like a professional coach. You correct crisply and keep things efficient, with less small talk.',
    },
    {
        id: 'zoe',
        name: 'Zoe',
        icon: '✨',
        voiceName: 'Aoede',
        style: 'You are upbeat, playful, and casual — like chatting with a fun friend. You correct lightly and keep the energy high.',
    },
    {
        id: 'reid',
        name: 'Professor Reid',
        icon: '🎓',
        voiceName: 'Fenrir',
        style: 'You are precise and academic, like a university professor. You explain corrections with a bit more grammatical detail than usual, but stay kind.',
    },
];
...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const earnedNow = new Set(achievements.badges.filter((b) => b.earned).map((b) => b.id));
        earnedNow.forEach((id) => {
            if (!prevBadgesRef.current.has(id) && prevBadgesRef.current.size > 0) {
                const badge = achievements.badges.find((b) => b.id === id);
                confetti({ particleCount: 100, spread: 70, origin: { y: 0.6 } });
                showToast(`${badge?.icon} Achievement unlocked: ${badge?.label}!`, 'success');
            }
        });
        prevBadgesRef.current = earnedNow;
    }, [achievements]);

    useEffect(() => {
        const met = weeklyCount >= weeklyGoal;
        if (met && !prevGoalMetRef.current && weeklyCount > 0) {
            confetti({ particleCount: 120, spread: 90, origin: { y: 0.6 } });
            showToast('🎉 Weekly goal reached!', 'success');
        }
        prevGoalMetRef.current = met;
    }, [weeklyCount, weeklyGoal]);

    useEffect(() => {
        if (!streak) return;
        const milestones = [3, 7, 14, 30, 100];
        if (milestones.includes(streak.streak) && streak.streak > prevStreakRef.current) {
            confetti({ particleCount: 80, spread: 60, origin: { y: 0.6 } });
            showToast(`🔥 ${streak.streak}-day streak!`, 'success');
        }
    ...
    async function startLiveSession() {
        try {
            const tokenRes = await fetch('/api/live-token', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ voiceName: selectedTutor.voiceName }),
            });
            ...
            const session = await liveClient.live.connect({
                ...
                config: {
                    ...
                    speechConfig: {
                        voiceConfig: { prebuiltVoiceConfig: { voiceName: selectedTutor.voiceName } },
                    },
                    systemInstruction:
                        `You are ${selectedTutor.name}, an English speaking conversation partner and tutor for a non-native speaker. ${selectedTutor.style} You are NOT a general assistant — never ask 'how can I help you' or offer to help with tasks. Instead, have a natural, warm spoken conversation about everyday life. Ask one engaging question at a time and react naturally. IMPORTANT: Every time the learner makes a grammar, tense, or word-choice mistake, briefly correct it before continuing, proactively, without being asked. Keep replies short, 1-3 sentences. STRICT RULE: Never discuss or entertain immoral, illegal, sexual, violent, or otherwise inappropriate topics, even if the learner brings them up — gently redirect to a wholesome topic instead.`,
                },
                callbacks: {
    ...
    return (
        <main className="min-h-screen relative overflow-hidden bg-gradient-to-br from-[#EEF2FF] via-[#F5F3FF] to-[#FDF4FF] dark:from-[#0F0B1E] dark:via-[#150F26] dark:to-[#1A1229] flex justify-center py-6 sm:py-14 pb-20 sm:pb-14 px-3 sm:px-4">
            <div className="fixed top-4 sm:top-6 left-1/2 -translate-x-1/2 z-50 flex flex-col gap-2 w-[90%] sm:w-auto max-w-sm">
                {toasts.map((t) => (
                    <div
                        key={t.id}
                        className="bg-white/95 dark:bg-[#1A1530]/95 backdrop-blur-xl border border-violet-200 dark:border-violet-700/40 rounded-full px-5 py-3 shadow-lg text-center font-sans text-[13.5px] text-[#1E1B2E] dark:text-violet-50 animate-[fade-slide-up_0.3s_ease-out]"
                    >
                        {t.message}
                    </div>
                ))}
            ...
            <div className="relative w-full max-w-2xl bg-white/70 dark:bg-[#1A1530]/80 backdrop-blur-xl rounded-2xl sm:rounded-[28px] shadow-[0_20px_60px_-15px_rgba(76,29,149,0.35)] border border-white/60 dark:border-violet-900/40 px-5 py-6 sm:px-14 sm:py-12">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="w-8 h-8 rounded-full bg-violet-50 dark:bg-violet-900/40 hover:bg-violet-100 dark:hover:bg-violet-900/60 flex items-center justify-center cursor-pointer transition-colors"
                            >
                            ...
                            {settingsOpen && (
                                <>
                                    ...
                                    <div className="absolute right-0 mt-2 w-44 bg-white dark:bg-[#1A1530] border border-violet-100 dark:border-violet-800/40 rounded-xl shadow-lg z-20 overflow-hidden">
                                        <button
                                            onClick={() => { setDarkMode(!darkMode); setSettingsOpen(false); }}
                                            className="w-full text-left px-4 py-2.5 text-sm text-[#1E1B2E] dark:text-violet-50 hover:bg-violet-50 dark:hover:bg-violet-900/30 cursor-pointer transition-colors flex items-center gap-2"
                                        >
                                            <span>{darkMode ? '☀️' : '🌙'}</span> {darkMode ? 'Light mode' : 'Dark mode'}
                                        </button>
                                        <button
                                            onClick={() => signOut({ callbackUrl: '/sign-in' })}
                                            className="w-full text-left px-4 py-2.5 text-sm text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 cursor-pointer transition-colors flex items-center gap-2 border-t border-violet-100 dark:border-violet-800/40"
                                        >
                                            <span>🚪</span> Sign out
                                        </button>
                                    </div>
                ...
                <div className="hidden sm:flex gap-1.5 sm:gap-2 mb-6 sm:mb-7 flex-wrap">
                    {MODES.map((m) => (
                        <button
                            key={m}
                            onClick={() => switchMode(m)}
                            className={`font-mono text-[10px] sm:text-xs uppercase px-3 sm:px-4 py-1.5 sm:py-2 rounded-full transition-colors cursor-pointer ${mode === m ? 'bg-violet-600 text-white' : 'bg-violet-50 dark:bg-violet-900/30 text-violet-500 dark:text-violet-300'
                                }`}
                        >
                            {m}
                        </button>
            ...
            <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-30 bg-white/90 dark:bg-[#1A1530]/95 backdrop-blur-xl border-t border-violet-100 dark:border-violet-800/40 px-1 py-1.5 flex overflow-x-auto gap-0.5">
                {MODES.map((m) => (
                    <button
                        key={m}
                        onClick={() => switchMode(m)}
                        className={`flex-1 min-w-[52px] flex flex-col items-center gap-0.5 py-1.5 rounded-xl cursor-pointer transition-colors ${mode === m ? 'text-violet-600 dark:text-violet-300' : 'text-[#9CA3AF] dark:text-violet-500/50'
                            }`}
                    >
                        <span className="text-base">{MODE_ICONS[m]}</span>
                        <span className="font-mono text-[8px] uppercase leading-none">{m}</span>
                    </button>
                ))}
            </nav>
        </main >
