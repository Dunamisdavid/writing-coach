    ...
    <div className="w-full max-w-sm bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl border border-white/60 p-8">
      {done ? (
        <>
          ...
          <button onClick={() => router.push('/sign-in')} className="text-violet-600 hover:text-violet-800 underline text-sm cursor-pointer transition-colors">
            Go to sign in
          </button>
        </>
      ) : (
        <form onSubmit={handleSubmit}>
          ...
          <input
            ...
            className="w-full bg-white/60 rounded-xl border border-violet-100 px-4 py-2.5 mb-4 text-sm text-[#1E1B2E] placeholder:text-[#9CA3AF] focus:outline-none focus:ring-2 focus:ring-violet-400"
            required
          />
